...
- **Fronts** : `for s in webiartisan-livry webiartisan-combs webiartisan-vert-saint-denis webiartisan-lieusaint; do make -C sites/$s push; done` — déploiement **atomique en 3 phases** (assets hashés sans delete → index.html en dernier → ménage >7 j). Puis comparer les hash `index-*.js` live vs `dist/assets/` pour chaque ville.
